...
- Dashboard *とターミナル*用の**31種類のアニメーション背景**（そう、`matrix` もある）。やりすぎかもしれないが、後悔はしていない。
...
Dashboardのビューごとに、*あるいは任意のターミナルの背後に*、**31種類**のキャンバスアニメーション背景から気分を選べる：

| 気分 | 背景 |
| --- | --- |
| 穏やか | `fuji`、`aurora`、`clouds`、`ocean`、`raindrops`、`rainywindow`、`frostedWindow`、`snow`、`sakura`、`fireflies`、`bubbles`、`aquarium`、`jellyfish`、`lighthouse`、`balloons`、`ricefield`、`lanterns` |
| 宇宙 | `starfield`、`nebula` |
| 暖 | `embers`、`lava`、`dunes`、`savanna` |
