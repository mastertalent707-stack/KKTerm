...
- **โมดูล IT Ops** ที่กระจายสคริปต์หรือ playbook แบบโต้ตอบออกไปยังโฮสต์ทั้งกลุ่มในคราวเดียว พร้อม **ระบบอัตโนมัติแบบ watchdog** ที่ ping เฝ้าดู และส่งอีเมลหาคุณเมื่อมีอะไรเพี้ยน
...
มันยังเห็นสิ่งที่คุณเห็นได้ด้วย: จับภาพเป็นพื้นที่หรือทั้ง Pane ด้วย **เมนูภาพหน้าจอ** แล้วส่งเข้าบทสนทนาตรง ๆ เพื่อให้ "ทำไมไดอะล็อกนี้หน้าตาแปลก ๆ?" กลายเป็นคำถามที่ผู้ช่วยตอบได้จริง ๆ
...
<p align="center">
  <img src="docs/assets/screenshots/usage-meter.png" alt="widget มาตรวัดการใช้ AI และตัวบ่งชี้บนแถบสถานะ" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>ที่วางภาพหน้าจอ</strong> — widget การใช้งานที่แสดง Claude Code และ Codex เคียงข้างกัน พร้อมภาพสะท้อนแบบกะทัดรัดบนแถบสถานะ</em></sub></p>

### ดูแลทั้งฟลีตโดยไม่ต้องออกจากหน้าต่าง

เมื่อ "เซิร์ฟเวอร์หนึ่งเครื่อง" กลายเป็น "เซิร์ฟเวอร์สี่สิบเครื่อง" เวิร์กโฟลว์แบบ pane เดียวก็เริ่มไม่ไหว **โมดูล IT Ops** คือที่ที่ KKTerm เติบโตขึ้น:

- **Host Groups (กลุ่มโฮสต์)** — ชุด Connection ที่ตั้งชื่อไว้ (พร้อมตัวกรองแบบไดนามิกเสริม) ใช้ซ้ำเป็นเป้าหมายสำหรับทุกอย่างด้านล่าง
- **Batch Runs (การรันเป็นชุด)** — กระจายสคริปต์ครั้งเดียวจบ *หรือ* **Playbook** แบบโต้ตอบสไตล์ expect ออกไปยังกลุ่มโฮสต์ผ่าน SSH, WinRM หรือ PsExec พร้อมความคืบหน้าแบบสดต่อโฮสต์และรายงานสรุปที่เปิดดูใหม่ได้
- **Automations และ Watchdog** — ตั้งกฎ (ping, การเข้าถึง TCP, performance counter หรือเซสชัน SSH ที่เงียบไป) แล้วเลือกว่าจะเกิดอะไรเมื่อมันทำงาน: แจ้งเตือน, ป็อปอัป, อีเมล, webhook, เริ่ม Batch Run หรือส่งต่อให้ AI กฎที่อยู่ทนรอดจากการรีสตาร์ต ส่วน Watchdog ที่ทำงานอยู่จะปรากฏบนแถบสถานะ

มันไม่ใช่ของแทน NOC มันคือเครื่องมือ "ต้องแพตช์เครื่องสิบสองเครื่องนี้ให้เสร็จก่อนมื้อเที่ยง" ที่คุณเฝ้าอยากให้เทอร์มินัลของคุณมีมาตลอด

<p align="center">
  <img src="docs/assets/screenshots/it-ops.png" alt="โมดูล IT Ops กำลังรัน batch run บนกลุ่มโฮสต์" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>ที่วางภาพหน้าจอ</strong> — กริด Batch Run ที่สตรีมเอาต์พุตต่อโฮสต์แบบสด พร้อมระบบอัตโนมัติ Watchdog ที่ตั้งพร้อมทำงานอยู่ในแผงข้าง ๆ</em></sub></p>
...
### ไฟล์และล็อก ในหน้าต่างเดียวกัน

ไม่ใช่ทุกอย่างจะเป็นโฮสต์รีโมต KKTerm เรียกดู **ดิสก์ในเครื่อง** ของคุณใน Pane File Explorer (เปลือกสองแผงแบบเดียวกับ SFTP) และเปิดไฟล์เดี่ยวในตัวดู **Document** ที่เลือกโหมดให้เหมาะกับงาน: ข้อความ/โค้ดพร้อมตัวแก้ไขแบบเบาและบันทึกอย่างปลอดภัย, Markdown, ตาราง CSV/TSV, JSON, รูปภาพ, PDF และ **โหมด Log** โดยเฉพาะที่มีการลงสีตามระดับ, ตัวกรอง, ANSI และ tail-follow ไม่ต้องไปหยิบตัวแก้ไขแยกต่างหากแค่เพื่ออ่านล็อกที่คุณยืนอยู่ข้าง ๆ อยู่แล้วอีกต่อไป

<p align="center">
  <img src="docs/assets/screenshots/file-viewer.png" alt="ตัวดู Document ในโหมด log-tail ข้าง pane file explorer" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>ที่วางภาพหน้าจอ</strong> — ตัวดู Document ที่กำลังตามล็อกแบบสด (สีตามระดับ + ตัวกรอง) ข้าง Pane File Explorer ในเครื่อง</em></sub></p>
...
<p align="center">
  <img src="docs/assets/screenshots/mcp-server.png" alt="การตั้งค่าเซิร์ฟเวอร์ MCP ในตัวพร้อมไดอะล็อกแสดงคอนฟิก" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>ที่วางภาพหน้าจอ</strong> — แผง Built-in MCP Server พร้อมไดอะล็อก "แสดงคอนฟิก" และสวิตช์ความปลอดภัย (ปิดเป็นค่าเริ่มต้น)</em></sub></p>
