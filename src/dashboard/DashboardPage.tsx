  ...
  useEffect(() => {
    ...
    onAssistantContextChange({
      ...
      text: [
        ...
        "Script widgets can request curated npm libraries via a body.libraries string array. The libraries load before the widget source and expose documented globals (see compact catalog below). Use these bundled libraries when the catalog covers the need (for example mermaid for diagrams, three for 3D scenes, animejs for property animation, echarts or chartjs for charts, marked for markdown, prism for syntax highlighting, leaflet for maps, qrcode for QR codes, mathjs for expressions, papaparse for CSV, dayjs for dates). Do not load runtime CDN scripts; permissions.network allows remote data and images, not remote code. If no catalog entry fits, prefer simple source code. Only list libraries you actually use; do not declare more than 8. The 64KB source budget applies to your code, not to library bytes.",
