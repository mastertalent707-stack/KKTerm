...
- **31 fondos animados** para el panel *y tus terminales* (sí, incluido `matrix`), porque no estamos por encima de eso.
...
Elige un ambiente — por vista del panel, *o detrás de cualquier terminal* — entre **31** fondos animados sobre canvas:

| Ambiente | Fondos |
| --- | --- |
| Calma | `fuji`, `aurora`, `clouds`, `ocean`, `raindrops`, `rainywindow`, `frostedWindow`, `snow`, `sakura`, `fireflies`, `bubbles`, `aquarium`, `jellyfish`, `lighthouse`, `balloons`, `ricefield`, `lanterns` |
| Espacial | `starfield`, `nebula` |
| Cálido | `embers`, `lava`, `dunes`, `savanna` |
