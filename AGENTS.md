...
- Native HWND-backed surfaces cannot be trusted to obey DOM z-index, but parking must be geometry-scoped. RDP ActiveX and WebView2 are parked/hidden only when an app-owned DOM overlay, dialog, menu, or region-selection surface intersects the native surface rectangle. Sidebar-only Quick Connect, Add Connection, tree context menus, and rail menus must not blank the active workspace when they do not overlap it. For RDP, preserve the screenshot/parking behavior in `src/remote-desktop/RemoteDesktopWorkspace.tsx`: capture the visible RDP host via the typed screenshot command, show that bitmap underneath the DOM overlay, then hide/park the ActiveX HWND until the overlay closes. Do not apply the RDP screenshot workaround to WebView2, terminal, SFTP, or VNC workspaces. Keep overlay detection centralized in `src/workspace/nativeOverlay.ts` and keep that helper surface-aware when adding new app-level overlays.
