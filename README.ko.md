...
- 예, `matrix` 포함, 아홉 가지 **애니메이션 캔버스 배경**. 우리도 그 정도 취향은 있습니다.
...
Dashboard에는 **Dashboard View**별로 선택할 수 있는 캔버스 애니메이션 배경이 아홉 가지 있습니다:

| 분위기 | 배경 |
| --- | --- |
| 차분한 | `aurora`, `raindrops` |
| 우주적인 | `starfield`, `nebula` |
| 따뜻한 | `embers`, `lava` |
| 덕후스러운 | `matrix`, `synthwave` |
| 혼란스러운 | `confetti` |

단일 공유 `requestAnimationFrame`으로 실행되며 창 포커스를 존중하므로, 다른 작업을 할 때는 거의 CPU를 소비하지 않습니다. `matrix`와 AI 어시스턴트를 조합하면 "나는 엄청나게 생산적이며 동시에 워쇼스키 영화 속에 있는 것 같기도 하다"는 분위기가 납니다. 아니면 `mist`를 골라서 진지한 사람처럼 보일 수도 있습니다. 어느 쪽이든 판단하지 않습니다.
...
| **Dashboard** | 지속 View, Widget 인스턴스, 편집 모드, 드래그/리사이즈, App Launcher, **AI 작성 콘텐츠/스크립트 Widget**(선언적 JSON 또는 권한이 있는 샌드박스 iframe JS), Widget별 프리셋 / 강조색 / 아이콘 / 제목, **9가지 애니메이션 캔버스 배경**(aurora, raindrops, starfield, nebula, embers, lava, matrix, synthwave, confetti) |
...
- **Dashboard Widget을 추가해 주세요.** 내장 Widget은 [`src/dashboard/widgets/`](src/dashboard/widgets/)에 있습니다. 작은 아이디어를 골라, 출시하고, 패턴을 익히세요.
