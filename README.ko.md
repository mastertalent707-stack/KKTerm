...
- **IT Ops 모듈**. 스크립트나 대화형 playbook을 호스트 그룹 전체에 한 번에 펼치고, ping하고·지켜보다가·뭔가 어긋나면 이메일로 알려주는 **watchdog 자동화**까지.
...
<p align="center">
  <img src="docs/assets/screenshots/hero.png" alt="KKTerm 전체 창: 연결 트리, 라이브 Pane 그리드, AI 어시스턴트" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>스크린샷 자리표시자</strong> — 작업 공간을 한눈에: 왼쪽 연결 트리, 가운데 라이브 Pane 그리드, 오른쪽 AI 어시스턴트.</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>스크린샷 자리표시자</strong> — 하나의 Tab에 여러 Connection 종류가 함께: SSH 옆에 SFTP, 그 옆에 내장 Web UI.</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>스크린샷 자리표시자</strong> — 4분할 그리드: PowerShell, SSH 세션, SFTP 브라우저, VNC 화면이 모두 하나의 Tab 안에.</em></sub></p>
...
당신이 보는 걸 AI도 볼 수 있다: **스크린샷 메뉴**로 영역이나 Pane 전체를 잡아 곧장 대화에 보내면, "이 대화상자가 왜 이렇게 보이지?"가 어시스턴트가 실제로 답할 수 있는 질문이 된다.

<p align="center">
  <img src="docs/assets/screenshots/ai-assistant.png" alt="도구 접근과 승인 모드 토글이 있는 AI 어시스턴트 패널" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>스크린샷 자리표시자</strong> — AI 어시스턴트 패널: 도구 묶음 토글, Prompt / Allow All 전환, 그리고 사람의 예스를 기다리는 위험한 명령.</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>스크린샷 자리표시자</strong> — AI가 만든 위젯으로 가득한 Dashboard View: ping 모니터, 포스트잇, 실시간 수치, 그리고 이렇게 재밌어도 되나 싶은 작은 장난감.</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>스크린샷 자리표시자</strong> — 기분 모음: `matrix`, `aurora`, `synthwave`, `taipei101`.</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>스크린샷 자리표시자</strong> — Pane 툴바의 tmux/psmux 세션 목록. 재연결 후에도 원격 `claude` 에이전트가 계속 돌고 있다.</em></sub></p>
...
<p align="center">
  <img src="docs/assets/screenshots/usage-meter.png" alt="AI 사용량 미터 위젯과 상태 표시줄 인디케이터" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>스크린샷 자리표시자</strong> — Claude Code와 Codex를 나란히 보여주는 사용량 위젯과, 간결한 상태 표시줄 미러.</em></sub></p>

### 창을 떠나지 않고 플릿 운영

"서버 한 대"가 "서버 마흔 대"가 되면 싱글 페인 워크플로는 더 이상 버티지 못한다. **IT Ops 모듈**은 KKTerm이 어른이 되는 곳이다：

- **Host Groups(호스트 그룹)** — Connection들의 이름 붙은 선택(선택적 동적 필터 포함). 아래 모든 기능의 대상으로 재사용된다.
- **Batch Runs(일괄 실행)** — 일회성 스크립트*나* 대화형 expect 방식의 **Playbook**을 SSH·WinRM·PsExec로 호스트 그룹에 펼친다. 호스트별 실시간 진행 상황과, 다시 열 수 있는 통합 리포트 포함.
- **Automations와 Watchdog** — 규칙(ping, TCP 도달성, 성능 카운터, 또는 SSH 세션이 조용해지는 것)을 무장시키고, 발동했을 때 무엇을 할지 고른다: 알림, 팝업, 이메일, webhook, Batch Run 시작, 또는 AI에게 맡기기. 규칙 자체는 재시작을 견뎌낸다; 라이브 Watchdog은 상태 표시줄에 나타난다.

NOC 대체재가 아니다. "점심 전에 이 열두 대에 패치를 해야 해"라는, 터미널에 늘 있었으면 했던 그 도구다.

<p align="center">
  <img src="docs/assets/screenshots/it-ops.png" alt="호스트 그룹에 대해 일괄 실행을 돌리는 IT Ops 모듈" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>스크린샷 자리표시자</strong> — 호스트별 출력을 실시간 스트리밍하는 Batch Run 그리드와, 그 옆에 무장된 Watchdog 자동화.</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>스크린샷 자리표시자</strong> — Activity Rail 상단의 워크스페이스 전환기. "Home Lab"과 "Day Job" 사이를 전환하는 중.</em></sub></p>

### 파일과 로그도, 같은 창 안에서

모든 게 원격 호스트인 건 아니다. KKTerm은 File Explorer Pane으로 **로컬 디스크**를 둘러보고(SFTP와 같은 듀얼 페인 셸), **Document 뷰어**로 단일 파일을 열어 용도에 맞는 모드를 자동으로 고른다: 경량 에디터와 안전 저장이 있는 텍스트/코드, Markdown, CSV/TSV 테이블, JSON, 이미지, PDF, 그리고 레벨 색상·필터·ANSI·tail 추적을 갖춘 전용 **Log 모드**. 바로 옆에 있는 로그를 읽겠다고 별도 에디터를 꺼낼 일은 이제 없다.

<p align="center">
  <img src="docs/assets/screenshots/file-viewer.png" alt="File Explorer 페인 옆에서 log tail 모드의 Document 뷰어" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>스크린샷 자리표시자</strong> — 실시간 로그를 추적하는 Document 뷰어(레벨 색상＋필터)와, 그 옆의 로컬 File Explorer Pane.</em></sub></p>
...
<p align="center">
  <img src="docs/assets/screenshots/mcp-server.png" alt="내장 MCP 서버 설정과 설정 표시 대화상자" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>스크린샷 자리표시자</strong> — Built-in MCP Server 패널. "설정 표시" 대화상자와 기본적으로 꺼진 안전 토글.</em></sub></p>
