...
<p align="center">
  <strong><a href="https://github.com/ryantsai/KKTerm/releases/latest">최신 KKTerm 릴리스 다운로드</a></strong>
</p>
...
- 로컬 디스크를 뒤지는 파일 관리자 하나, 그리고 계속 tail 중인 그 로그 하나 때문에 켜둔 텍스트 에디터
...
- Dashboard *와 터미널*용 **애니메이션 배경 31종**(그래, `matrix`도 있다). 좀 과하긴 한데, 후회는 없다.
...
<p align="center"><sub><em>(데모 GIF. 한 장의 그림이 천 개의 글머리표를 이긴다. 그리고 글머리표는 거의 다 떨어졌다.)</em></sub></p>
...
| 내 디스크 둘러보기 | 로컬 File Explorer 페인, SFTP와 같은 듀얼 페인 셸 |
| 로그·CSV·이미지·PDF 열기 | 진짜 tail 추적 로그 모드를 갖춘 내장 Document 뷰어 |
| 호스트 CPU 보기 | 실시간 상태 표시줄과, 직접 쌓아 올릴 수 있는 Dashboard |

같은 앱. 같은 창. 같은 단축키. 같은, 부디 눈이 아프지 않기를 바라는 테마.

<p align="center">
  <img src="docs/assets/screenshots/connections-grid.png" alt="하나의 Tab에 SSH, SFTP, 내장 Web UI를 나란히" width="720" />
</p>
...
KKTerm은 플랫폼이 아니라 도구처럼 느껴지도록 만든다. 현재 데스크톱 빌드는 20 MB 미만이고, 설치가 빠르며, 관리자 작업 공간을 여는 일이 두 번째 운영체제를 부팅하는 것처럼 느껴지지 않을 만큼 빠르게 실행된다.
...
<p align="center">
  <img src="docs/assets/screenshots/multi-pane.png" alt="하나의 Tab을 네 개의 서로 다른 연결 종류의 페인으로 분할" width="720" />
</p>


### 당신의 터미널을 조종해 주는 AI 어시스턴트

대부분의 "터미널 속 AI" 데모는 채팅에서 멈춘다. KKTerm의 어시스턴트는 당신의 세션 *안에서* 동작한다: 화면에 이미 있는 것을 컨텍스트로 건네면, 어시스턴트가 당신이 연결된 머신에 직접 손을 댄다 — 게다가 사람이 늘 승인 루프 안에 있다.

**컨텍스트를 곧바로 건넨다.** 복사·붙여넣기로 옮길 필요 없다:

- **터미널 버퍼를 컨텍스트에 추가**하면 돌아가는 로컬 또는 원격 세션의 스크롤백이 곧장 대화로 들어와, "이 빌드가 왜 실패했지?"가 어시스턴트가 실제로 읽을 수 있는 질문이 된다.
- **스크린샷 메뉴**로 영역이나 Pane 전체를 잡아 이미지를 대화에 떨어뜨리면, "이 대화상자가 왜 이렇게 보이지?"에 답할 수 있다.
- **파일 첨부**와 현재의 **Dashboard / IT Ops 페이지 컨텍스트**도 건네면, 모호한 설명이 아니라 당신이 실제로 보고 있는 것을 바탕으로 추론한다.

**손을 대게 한다 — 단, 승인 뒤에서.** 어시스턴트는 당신의 터미널에서 명령을 실행하고, Connection을 열고, Dashboard에 위젯을 놓을 수 있지만, 위험한 부분은 게이트 뒤에 남는다:
...
- `rm -rf`처럼 보이는 건 전부 위험으로 표시되어 — 승인 카드에 그 이유가 표시되고 — 사람의 명시적인 예스를 기다린다. 누군가 man page에 프롬프트 인젝션을 심어 잔머리를 굴려도, AI가 파괴적인 명령을 몰래 실행하는 일은 없다.

**두뇌는 직접 가져온다.** OpenAI, Anthropic, OpenRouter, DeepSeek, Grok, Azure OpenAI, LiteLLM, GitHub Copilot, Ollama, NVIDIA, 또는 OpenAI 호환 엔드포인트 무엇과도 대화한다 — 게다가 **Claude Code CLI**나 **Codex CLI**를 백엔드로 돌려, 별도의 API 키 대신 기존 `claude` / `codex` 로그인과 구독을 그대로 쓸 수 있다. API 키는 OS 키체인에 들어간다.

<p align="center">
  <img src="docs/assets/screenshots/ai-assistant.png" alt="도구 접근과 승인 모드 토글이 있는 AI 어시스턴트 패널" width="720" />
</p>
...
Dashboard 뷰마다, *또는 아무 터미널 뒤에*, **31종**의 캔버스 애니메이션 배경에서 기분을 고를 수 있다：

| 기분 | 배경 |
| --- | --- |
| 잔잔 | `fuji`, `aurora`, `clouds`, `ocean`, `raindrops`, `rainywindow`, `frostedWindow`, `snow`, `sakura`, `fireflies`, `bubbles`, `aquarium`, `jellyfish`, `lighthouse`, `balloons`, `ricefield`, `lanterns` |
| 우주 | `starfield`, `nebula` |
| 따뜻 | `embers`, `lava`, `dunes`, `savanna` |
...
<p align="center">
  <img src="docs/assets/screenshots/backgrounds.png" alt="여러 애니메이션 배경을 나란히" width="720" />
</p>
...
로컬 셸도 Windows에서 같은 묘기를 부린다: PowerShell 페인은 **psmux**(네이티브 tmux 클론) 안에서 돌 수 있어, 로컬의 장시간 작업도 원격과 똑같이 Pane을 닫아도 살아남는다.

<p align="center">
  <img src="docs/assets/screenshots/tmux-reattach.png" alt="재연결 후 이름 붙은 tmux 세션에 다시 붙는 SSH 페인" width="720" />
</p>


### 워크스페이스로 세계를 분리하기

홈랩, 본업, 그리고 그 고객사 서버는 같은 목록에 있을 게 아니다. **워크스페이스(Workspaces)**는 Activity Rail에서 전환하는, 이름 붙고 격리된 Connection 컨테이너다. 전환해도 다시 스코프되는 건 연결 트리뿐 — 열려 있는 Sessions, Dashboard, 설정은 그대로 — 그래서 맥락 전환은 재시작이 아니라 클릭 한 번이면 된다.

<p align="center">
  <img src="docs/assets/screenshots/workspaces.png" alt="Activity Rail의 워크스페이스 전환기" width="720" />
</p>


### 취향대로 입히기: 컬러 테마

배경이 재미있는 부분이라면, **컬러 테마**는 하루 종일 실제로 들여다보는 부분이다. KKTerm은 앱 전체 크롬 — Activity Rail, 연결 트리, 탭, 대화상자 — 을 다시 칠하는 **26종**의 컬러 스킴을 제공하며, 설정 ▸ 모양에서 각 스킴의 실시간 미니 미리보기를 보여준다：

| 분위기 | 스킴 |
| --- | --- |
| 뉴트럴 | `Default`, `Dark`, `Light`, `Match OS`(시스템 라이트/다크를 따름), `Mac` |
| 컬러풀 | `Orange`, `Purple`, `Pink`, `Confetti`, `Bubble Tea` |
| 로컬 감성 | `Green Kuai Kuai`(그래, 그 과자), `Blue See`, `Blue, Green and White`, `Semiconductor` |

어느 스킴을 골라도 터미널은 자체 다크 팔레트를 유지하므로, 셸은 읽기 좋은 상태를 지키고 앱의 나머지는 당신의 기분에 맞춘다.

<p align="center">
  <img src="docs/assets/screenshots/color-themes.png" alt="실시간 미리보기가 있는 설정의 컬러 스킴 그리드" width="720" />
</p>


### Install Helper (Windows 전용)

개발용으로 새 Windows 머신을 세팅하는 건 보통 브라우저 탭 열 개와 "다음, 다음, 완료"의 반복이다. **Install Helper**는 내장 도구 카탈로그로, 평소엔 손으로 쫓아다니던 도구를 KKTerm을 떠나지 않고 찾고·설치하고·업데이트하고·제거한다：

- **Essentials(필수)** — winget, Node(nvm-windows 경유), Python(uv 경유), Git.
- **AI Agents** — Claude Code, Codex, Antigravity, OpenCode, 그 외 코딩 에이전트 CLI와 데스크톱 앱.
- **AI Platforms** — Ollama, n8n, Open WebUI, Flowise, Langflow 같은 로컬／셀프호스트 스택을 실행·관리까지 대신.
- **Development(개발)** — 에디터, 컨테이너, API 도구, WSL과 그 배포판, Rustup.
- **Windows Power User** — PowerToys, PowerShell 7, psmux, Sysinternals, Everything, Ditto.
- **Remote Access(원격 접속)** — Tailscale, RustDesk.
- **Utilities(유틸리티)** — Notepad++, ripgrep, jq, fzf, 7-Zip, Oh My Posh, FFmpeg 등.

이미 설치된 것을 감지하고, 업데이트가 있는 것에 표시하며, **모두 업데이트**가 큐 전체를 대신 처리한다. UAC 프롬프트는 명시적으로 유지되고, 무엇도 조용히 설치되지 않으며, 카탈로그 전체가 앱에 동봉되어 있다 — 추가 계정도, 백그라운드 텔레메트리도 없다.

> macOS와 Linux에는 이미 즐겨 쓰는 패키지 매니저가 있으므로, Install Helper는 Windows 전용 편의 기능이며 해당 빌드에는 들어 있지 않다.

<p align="center">
  <img src="docs/assets/screenshots/install-helper.png" alt="설치됨과 설치 가능 도구가 있는 Install Helper 카탈로그" width="720" />
</p>
...
- **모든 OS가 같다고 가장하지 않는다.** KKTerm은 Windows, macOS, Linux 빌드를 배포하지만, 플랫폼별 기능은 정직하게 구분한다. Windows에는 네이티브 RDP ActiveX 경로와 Install Helper 카탈로그가 있고, macOS와 Linux는 해당 시스템에서 사용할 수 있는 포터블 경로를 사용한다.
...
**[최신 KKTerm 릴리스를 다운로드](https://github.com/ryantsai/KKTerm/releases/latest)** 하고, 사용하는 플랫폼에 맞는 패키지를 골라 열면 된다. Windows 설치 프로그램은 현재 **서명되지 않았다** — 릴리스 서명은 로드맵에 있으므로, 그전까지는 백신이 당신을 엄한 눈으로 볼 수 있다. 정상이다.
...
- [사용자 매뉴얼](docs/manual/INDEX.md) — 기능별 안내
- [로드맵](docs/ROADMAP.md)
- [Dashboard 아키텍처](docs/DASHBOARD.md)
- [내장 MCP 서버](docs/MCP.md)
