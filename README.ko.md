...
- Dashboard *와 터미널*용 **애니메이션 배경 25종**(그래, `matrix`도 있다). 좀 과하긴 한데, 후회는 없다.
...
Dashboard 뷰마다, *또는 아무 터미널 뒤에*, **25종**의 캔버스 애니메이션 배경에서 기분을 고를 수 있다：
...
배경이 재미있는 부분이라면, **컬러 테마**는 하루 종일 실제로 들여다보는 부분이다. KKTerm은 앱 전체 크롬 — Activity Rail, 연결 트리, 탭, 대화상자 — 을 다시 칠하는 **14종**의 컬러 스킴을 제공하며, 설정 ▸ 모양에서 각 스킴의 실시간 미니 미리보기를 보여준다：
