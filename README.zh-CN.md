...
Dashboard 提供九种可按 **Dashboard View** 独立选择的 Canvas 动态背景：
...
它们运行在单个共享的 requestAnimationFrame 上，并尊重窗口焦点，所以你去干别的事的时候它们几乎不消耗资源。用 `matrix` 配上你的 AI 助手，打造出一种"我极度高效，同时可能正身处沃卓斯基电影中"的氛围。或者选 `mist`，看起来像个严肃的人。我们两种选择都不评判。
...
- **添加一个 Dashboard Widget。** 内置 Widget 位于 [`src/dashboard/widgets/`](src/dashboard/widgets/)。想一个小点子，发布它，学习这个模式。
