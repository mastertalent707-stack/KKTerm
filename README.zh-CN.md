...
- Dashboard *和你的终端*用的**31 种 canvas 动画背景**（对，包括 `matrix`），因为我们也没在客气。
...
可以挑一种心情 — 给每个 Dashboard view，*或是放在任何终端后面* — 从 **31 种** canvas 动画背景里选：

| 心情 | 背景 |
| --- | --- |
| 平静 | `fuji`、`aurora`、`clouds`、`ocean`、`raindrops`、`rainywindow`、`frostedWindow`、`snow`、`sakura`、`fireflies`、`bubbles`、`aquarium`、`jellyfish`、`lighthouse`、`balloons`、`ricefield`、`lanterns` |
| 太空 | `starfield`、`nebula` |
| 温暖 | `embers`、`lava`、`dunes`、`savanna` |
