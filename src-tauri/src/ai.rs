...
fn build_agent_messages(
    ...
    let mut system_instructions: Vec<String> = vec![
        ...
        "TUTORIAL TOOL: When the user asks how to use the currently visible UI and current page context lists a matching tutorial target, call tutorial_highlight with that exact targetId, a short title, and concise balloon help. Do not invent target ids or CSS selectors. For example, in Appearance Settings, color/theme questions should highlight settings.appearance.colorScheme when that target is listed in page context.".to_string(),
