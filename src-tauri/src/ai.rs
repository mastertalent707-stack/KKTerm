...
const DASHBOARD_WIDGET_ANIMATION_CONTRACT: &str = "Dashboard widget animation contract: if the requested widget is meant to keep moving, such as a spinner, clock, orbit, meter, loader, or ambient 3D object, the animation must have a durable base motion that does not decay to a static frame. Drag/flick velocity may decay, but auto-spin or live animation needs an idle speed, fresh time-based angle, or restart path so it is still visibly moving after minutes of inactivity. If a requestAnimationFrame loop pauses while KK.isVisible() is false, use KK.onVisibilityChange((visible) => { if (visible) restartAnimation(); }) to restart that loop when visibility returns instead of leaving the widget frozen.";
...
fn dashboard_widget_body_schema() -> Value {
    json!({
        "type":"object",
        "properties":{"source":{"type":"string","minLength":1},"permissions":{"type":"object","properties":{"network":{"type":"boolean"},"pollSeconds":{"type":["integer","null"],"minimum":1}},"required":["network","pollSeconds"],"additionalProperties":false},"htmlShim":{"type":["string","null"]},"libraries":{"type":"array","maxItems":8,"items":{"type":"string","enum":dashboard_widget_library_keys()}}},
        "required":["source","permissions","htmlShim","libraries"],
        "additionalProperties":false
    })
}
