...
- **English value**: `Auth secret (stored in keychain)`
- **Namespace**: `settings`
- **File/component**: `src/settings/McpServers.tsx` and `src/settings/AddMcpServerDialog.tsx`
- **UI role**: `label`
- **User flow**: Editable masked input where the user confirms or replaces the detected token before it goes to keychain.
