...
- `src/modules/workspace/connections/webview/WebViewWorkspace.tsx` — URL Connection workspace shell, external-open session request, toolbar navigation affordances, status, and disabled embedded-browser placeholder.
- `src/modules/workspace/connections/webview/webview.css` — URL workspace toolbar, address input, status, and placeholder CSS.
...
- `webview.rs` — stubbed URL Connection backend: validates http(s) targets, opens them in the user's default browser, preserves the command surface as no-ops, and owns the main WebView clipboard-permission helper.
