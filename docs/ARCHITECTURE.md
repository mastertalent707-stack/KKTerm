...
Assistant tools are registered in `src-tauri/src/ai.rs` with JSON schemas and are filtered by persisted `assistantTools` settings. Tool families currently include general utility tools, Dashboard tools, saved Connection tools, and live Session tools. Saved Connection tools operate on durable SQLite Connection data and may list, create, open, update, or delete saved Connections. Live Session tools operate on currently mounted workspace surfaces only: terminal Panes expose visible buffer reads and input writes; RDP/VNC Sessions expose screenshots, text injection, named keypresses, and mouse clicks; SFTP/FTP browser Sessions expose listing plus create-folder, rename, and delete operations. Do not put live Session state into the durable Connection model to make an assistant tool easier to call.
...
- `src/dashboard/registry/presetRegistry.ts` — five preset chrome components (Panel, Ambient, Tile, Hero, Action). Presets read `--w-accent` / `--w-accent-soft` set by `WidgetFrame`.
