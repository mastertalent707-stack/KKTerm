...
- `src/modules/settings/GeneralSettings.tsx` — Language (i18n) selector, Auto Backup toggle and last-backup status, connected Connection rail shortcut toggle, minimize-to-tray toggle, DirectX screen capture toggle, settings backup/import actions, database folder opener.
...
- **Three widget kinds.** `builtIn` (TypeScript components in `src/modules/dashboard/widgets/`, registered in `src/modules/dashboard/registry/builtInRegistry.ts`), `content` (declarative JSON: markdown/kvList/checklist/stat; markdown-shaped content supports `data.mode: markdown | html`), and `script` (JavaScript executed inside an isolated `iframe srcdoc` host with declared `network` / `pollSeconds` permissions). Built-in widgets ship with the app; `content` and `script` widgets are AI Created Widgets in v1 and stored in `dashboard_custom_widgets`.
...
- `src/modules/workspace/workspace.css` — tab strip, workspace canvas, screenshot menu/Region overlay, bottom Status Bar, and screenshot gallery CSS.
...
- `src/modules/dashboard/schema.ts` — TypeScript validator for AI Created content/script widget body JSON before browser-preview persistence and render.
...
- `src/modules/dashboard/view/WidgetBody.tsx` — dispatcher: `builtIn` → registry lookup; `content` → `ContentWidgetRenderer`; `script` → `ScriptWidgetHost`.
- `src/modules/dashboard/widgets/` — one file per built-in widget body. `AppLauncherBody.tsx` delegates to `src/modules/dashboard/widgets/builtin/app-launcher/`.
...
- `src/modules/dashboard/content/ContentWidgetRenderer.tsx` — declarative `content`-kind renderer; switches over `shape: 'markdown' | 'kvList' | 'checklist' | 'stat'`, parsing Markdown or sanitizing HTML according to markdown-shaped `data.mode`.
