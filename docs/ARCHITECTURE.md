...
- **Managed web UI affordances** are allowlisted per tool. Installed n8n, Flowise, Open WebUI, Langflow, Excalidraw, BentoPDF, and Ollama expose Run/Stop/Open web UI actions with app-local commands and localhost URLs. BentoPDF prefers `localhost:3022` but its app-local server records the actual free port if that port is already occupied, and Open web UI uses the recorded URL. After install/update, these managed web UI apps start automatically in normal run mode. Stop is also allowlisted per tool port (or the recorded dynamic port) and, when a registered service exists, routes through the service helper. Arbitrary catalog commands are not accepted.
...
- **Managed Windows service helpers** are allowlisted per tool and backed by NSSM. Installed n8n, Flowise, Open WebUI, Langflow, Excalidraw, BentoPDF, and Ollama expose an explicit Register as service action that first stops the normal localhost run for that tool when KKTerm has a recorded managed port, then registers and starts a `KKTerm-*` service with the same managed app-local command and environment, writes NSSM stdout/stderr to `%LOCALAPPDATA%\KKTerm\installer\apps\<tool_id>\logs\`, and sets startup to automatic. BentoPDF keeps the same dynamic-port fallback under the service path instead of killing an unrelated process that already owns its preferred port. Recipes that expose service registration declare `nssm` as a prerequisite so the wrapper is installed before the managed server app. The helper may trigger UAC; arbitrary catalog commands are not accepted.
