...
- **Five visual presets.** `panel`, `ambient`, `tile`, `hero`, `action`. Each preset is a CSS chrome wrapper that reads the widget's `--w-accent` / `--w-accent-soft` variables; presets do not encode their own palette. Ambient supports an optional frosted-glass background; Action supports layout direction (vertical/horizontal).
