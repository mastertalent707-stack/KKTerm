...
- **Six visual presets.** `panel`, `ambient`, `tile`, `hero`, `mono`, `action`. Each preset is a CSS chrome wrapper that reads the widget's `--w-accent` / `--w-accent-soft` variables; presets do not encode their own palette. Ambient supports an optional frosted-glass background; Action supports layout direction (vertical/horizontal).
...
- `src/dashboard/registry/presetRegistry.ts` — six preset chrome components (Panel, Ambient, Tile, Hero, Mono, Action). Presets read `--w-accent` / `--w-accent-soft` set by `WidgetFrame`.
