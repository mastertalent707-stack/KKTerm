...
- **Winget CLI utilities ensure PATH.** NSSM, ripgrep, jq, fzf, and uv add `%LOCALAPPDATA%\Microsoft\WinGet\Links` to the persisted user PATH after install so new shells and follow-up helpers can resolve their command shims. Bundle follow-up commands and manager-backed runtime detection refresh the persisted Windows user and machine environment, including PATH, manager-specific variables such as `NVM_HOME` / `NVM_SYMLINK`, and existing Git for Windows command directories, before spawning.
