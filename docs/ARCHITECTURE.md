...
Assistant tools are registered in `src-tauri/src/ai.rs` with JSON schemas and are filtered by persisted `assistantTools` settings. Tool families currently include general utility tools, Dashboard tools, saved Connection tools, live Session tools, and the app-owned Tutorial overlay tool. Saved Connection tools operate on durable SQLite Connection data and may list, create, open, update, or delete saved Connections. Live Session tools operate on currently mounted workspace surfaces only: terminal Panes expose visible buffer reads and input writes; RDP/VNC Sessions expose screenshots, text injection, named keypresses, and mouse clicks; SFTP/FTP browser Sessions expose listing plus create-folder, rename, and delete operations. The Tutorial tool may only target frontend-owned `data-tutorial-id` anchors listed in current page context; it does not accept arbitrary CSS selectors. Do not put live Session state into the durable Connection model to make an assistant tool easier to call.
...
Debug builds also write raw AI Assistant interaction records to `aiassistant.debug.log` in the same directory as `kkterm.log`. This JSONL log is for local troubleshooting only and is intentionally more verbose than `[kkterm-ai]` console diagnostics: it records assistant run start/end, raw provider request/response bodies, raw SSE stream chunks, emitted stream events, tool calls/results, permission-required blocks, live Session bridge requests/completions/results, GitHub Copilot SDK prompt/response/error, and `dashboard_create_widget` checkpoints before and after custom-widget and instance persistence. Use it to distinguish model decisions from KKTerm harness, permission, Dashboard validation, and widget persistence failures. Do not enable this log in release builds, do not add visible in-app indicators for it, and treat the file as sensitive because raw prompts, screenshots/data URLs, file attachment text, terminal buffer reads, tool outputs, and generated widget source can appear in it. API keys and keychain secret values should not be logged deliberately, but users must still review the file before sharing it.
...
- `src/app/TutorialOverlay.tsx` — app-owned guided-help overlay used by the AI Assistant Tutorial tool. It resolves constrained `data-tutorial-id` anchors, dims the rest of the app, places a short balloon, and dismisses on the next user interaction. Because it can intersect RDP, it participates in `src/workspace/nativeOverlay.ts` for RDP ActiveX parking only.
...
- `src/ai/AssistantPanel.tsx` — AI Assistant chat surface, markdown rendering, chat history, extension draft intent UI, terminal send handoff, tool permission mode UI, and frontend completion of live Session tool requests.
