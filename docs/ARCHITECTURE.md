...
- **Six recipe providers, all pure data.** `winget`, `npm`, `githubRelease`, `windowsFeature`, `wslDistro`, `bundle`. No `custom`, no script strings — a tool that does not fit one of these six shapes does not ship in the catalog.
...
- **Managed server apps** live under `%LOCALAPPDATA%\KKTerm\installer\apps\<tool_id>\`. n8n installs as a local npm project with `N8N_USER_FOLDER` pointed at its app-local `data` directory. Ollama installs into an app-local `app` directory where supported by the Windows installer and runs with `OLLAMA_MODELS` pointed at its app-local `data\models` directory.
...
- **Managed web UI affordances** are allowlisted per tool. Installed n8n exposes Run (`npm exec --prefix ... n8n start`) and Open web UI (`http://localhost:5678`) actions; installed Ollama exposes Run (`ollama serve`) and Open web UI (`http://localhost:11434`) actions. Arbitrary catalog commands are not accepted.
