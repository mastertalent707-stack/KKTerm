...
RDP ActiveX is a native child HWND, not a DOM element. It can draw above React dialogs and menus regardless of CSS `z-index`; this is the Win32/WPF "airspace" limitation, not a stacking-context bug. When a DOM overlay must appear above an active RDP view, `src/workspace/nativeOverlay.ts` detects whether a registered overlay actually intersects the RDP host rectangle. Only then does `src/remote-desktop/RemoteDesktopWorkspace.tsx` capture the current RDP host rectangle through the screenshot command, render that transient bitmap inside the workspace, and hide/park the ActiveX HWND until the overlay is gone. This preserves the user's visual context for dialogs, screenshot menus, Region selection, and app-owned menus that geometrically cross the RDP host without blanking the active workspace for sidebar-only menus.

Do not turn overlay detection back into a document-wide boolean. Quick Connect, Add Connection, tree context menus, rail menus, and other sidebar flyouts must not suppress RDP or WebView2 unless their rendered rectangle intersects the native surface. RDP is the only workspace family that uses the screenshot-backed parking workaround. WebView2 may be visibility-parked when an intersecting overlay must stack above it, but it must not use the RDP screenshot substitution path. Terminal, SFTP, VNC, and other DOM/canvas-rendered workspaces should stay live and visible for ordinary app-owned menus; if they have a stacking bug, fix their stacking/layout directly rather than adding them to native parking.
...
- `src/workspace/nativeOverlay.ts` — shared geometry-based overlay suppression detection for native HWND-backed surfaces; update this when a new DOM menu, dialog, or overlay may intersect WebView2 or RDP. Keep the detector surface-aware, not document-wide.
...
- `src/webview/WebViewWorkspace.tsx` — URL Connection WebView2 host, geometry-scoped WebView2 visibility parking for intersecting overlays, webview session lease management, toolbar navigation, credential fill.
