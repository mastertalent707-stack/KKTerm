...
- Export / import (Custom tab only): each Custom Widget card has an export action (`dashboard.exportCustomWidget`); the Custom tab also offers `dashboard.exportAllWidgets` and `dashboard.importWidget`. Export writes a portable `.kkwidget` JSON file (filter `dashboard.widgetFileFilter`) containing the widget definition only — never instance secrets. Import opens a dialog: users can load a `.kkwidget` file (`dashboard.importWidgetFromFile`) or paste JSON directly (`dashboard.importWidgetPasteLabel`), then KKTerm validates it and shows a preview (`dashboard.importWidgetPreviewTitle`) before the user confirms. Import is additive: it adds each widget as a new user-authored Custom Widget with a fresh id, suffixing the title on collision, and never overwrites existing widgets. Status `dashboard.exportWidgetsComplete` / `dashboard.importWidgetsComplete`. Built-in widgets are not exportable.
