...
Previously opened Views remain mounted while hidden so switching between View tabs preserves Widget Instance state and keeps embedded Connection panes responsive; hidden Connection panes are marked inactive instead of being closed. Embedded URL Connection panes currently own no native WebView2 child surface because the URL backend is stubbed without Tauri's `unstable` feature; URL Connections open in the user's default browser.
...
While the `dashboard.changeBackground` picker is open, embedded URL Connection widgets on the active View may still be marked inactive, but there is currently no live URL WebView2 surface to hide. Keep any future native-surface suppression owned by the affected widget or surface rather than adding Dashboard-wide overlay parking.
