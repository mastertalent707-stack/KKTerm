...
- SSH: `settings.defaultUser`, `settings.defaultPort`, `settings.defaultKey`, `settings.proxyJump`, `settings.sshSocksProxy`, `settings.sshSocksProxyUsername`, `settings.sshSocksProxyPassword`, `settings.sshBufferLines`.
...
- Settings data subsection (destructive actions live here):
  - Export: `settings.exportSettings` opens the selective export dialog with a switch per category (`settings.segment_connections`, `settings.segment_workspaces`, `settings.segment_dashboards`, `settings.segment_settings`, `settings.segment_mcpServers`) and an off-by-default `settings.includeCredentials` switch that, when enabled, requires a passphrase (`settings.exportPassphrase`) and warns via `settings.includeCredentialsWarning`. Produces a `.kkbackup` file; success `settings.selectiveExportComplete`.
  - Import: `settings.importSettings` opens the backup import dialog. Selecting a `.kkbackup` inspects the bundle and shows a per-category action selector — `settings.importActionSkip` / `settings.importActionAdd` / `settings.importActionReplace` — plus a passphrase field for encrypted bundles. Selecting a full `.zip` backup shows `settings.fullBackupImport` and restores the complete settings database through `settings.importSettingsComplete`, then reloads the app. See [17-data-backup-secrets.md](17-data-backup-secrets.md).
...
- Debug subsection `settings.debug`: toggle `settings.advancedDebugging` (hint `settings.advancedDebuggingHint`) enables full AI Assistant, MCP, Install Helper, UI, URL Connection, RDP, SSH, and heartbeat debug log writing even in release builds. `settings.openLogFolder` opens the local log directory. KKTerm resolves that directory from the running `kkterm.exe` folder first and falls back to `%LOCALAPPDATA%\KKTerm\Logs` if the executable folder cannot be resolved. Enabling advanced debugging writes an `advanced_debugging.enabled` marker to the JSONL logs `aiassistant.debug.log`, `mcp.debug.log`, `installer.helper.debug.log`, `ui.debug.log`, `url.connection.debug.log`, `rdp.debug.log`, and `ssh.debug.log` beside `kkterm.log`, so the release logging path is visible before the next assistant, MCP, Install Helper, UI, URL Connection, RDP, or SSH event. It also starts `kkterm-heartbeat.debug.log`; turning the setting off stops release heartbeat writes. The `ui.debug.log` records frontend UI diagnostics such as terminal focus-restore attempts (document focus state and the active element) used to troubleshoot input focus after switching apps. The `url.connection.debug.log` records URL Connection Session lifecycle, frontend DOM bounds, clipping, requested overlay bounds, native coordinate conversion, and WebView2 overlay window/client rectangles. The `rdp.debug.log` records RDP startup, ActiveX control creation, display-size sync, and main-thread command timing with non-secret Connection details, and defensively redacts password-like, secret-like, token-like, and credential-like fields. The `ssh.debug.log` records native SSH lifecycle, SOCKS proxy routing metadata with credentials stripped, keepalive configuration, channel open/close events, and terminal input/output byte counts without terminal contents. These local logs may include prompts, tool arguments, MCP arguments/results, Install Helper command output, URL hostnames, remote hostnames, usernames, proxy endpoints, local paths, frontend/native liveness timing, screenshots, and generated Dashboard AI Created Widget source.
...
- Secret storage selector: `settings.credentialStorage` / `settings.credentialStorageBackend`. Windows and macOS default to `settings.credentialStorageOs` and can optionally switch to `settings.credentialStorageFile`; Linux exposes only `settings.credentialStorageFile`. The Credentials page shows a backend-specific security reminder: OS keystore storage uses the operating system credential manager and signed-in account, while encrypted database storage keeps encrypted rows in SQLite and depends on the master password plus database-file protection. Selecting `settings.credentialStorageFile` opens the setup dialog `settings.encryptedSecretStoreSetupTitle`, which repeats the security tradeoff before password entry. If no encrypted SQLite secret store exists, the dialog creates a master password and reminds the user to keep it safe because KKTerm cannot recover it. If one already exists, the dialog verifies the entered master password and unlocks the store. Linux users see a reminder that encrypted database storage is the available backend in this build and should protect the KKTerm database, backups, and `KKTERM_SECRET_STORE_PASSWORD` carefully. On later app launches, encrypted database storage stays locked until the first feature that needs to read a saved secret, such as opening a Connection with a saved password, or until the user activates the Status Bar `app.credentialStoreUnlockAction`. The prompt is the encrypted database dialog only; it does not navigate to Settings first. Setting `KKTERM_SECRET_STORE_PASSWORD` before launch lets KKTerm try to unlock the encrypted SQLite store automatically; if verification succeeds, the user does not need to type the master password for that app launch. If decryption fails, the dialog can create a new master password, which invalidates and clears encrypted credentials saved with the previous password. The switch changes which backend KKTerm reads and writes from. It does not migrate existing secrets between stores.
