...
- Settings data subsection (destructive actions live here):
  ...
  - Import: `settings.importSettings` opens the selective import dialog, inspects the chosen `.kkbackup`, and shows a per-category action selector — `settings.importActionSkip` / `settings.importActionAdd` / `settings.importActionReplace` — plus a passphrase field for encrypted bundles. A safety backup runs first (`settings.selectiveImportWarning`); success `settings.selectiveImportComplete`. See [17-data-backup-secrets.md](17-data-backup-secrets.md).
...
> Automatic database backups do **not** run from app-window close. Startup backups remain full ZIP snapshots; the visible Settings Import/Export buttons use `.kkbackup`.
