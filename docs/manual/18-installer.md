...
- **AI Platforms** (`installer.section.aiPlatforms`) — local or self-hosted AI platforms such as Ollama and n8n. These are managed server apps under `%LOCALAPPDATA%\KKTerm\installer\apps\<tool_id>\` rather than ordinary global command-line tools.
- **Development** (`installer.section.development`) — editors, containers, API tools, base WSL, and WSL distribution shortcuts.
- **Utilities** (`installer.section.utilities`) — Notepad++, NSSM, ripgrep, jq, and fzf. Winget-installed command-line utilities add the winget links directory (`%LOCALAPPDATA%\Microsoft\WinGet\Links`) to the user PATH so new shells can find their commands.

Each tool surface is a **tile** in the section grid. Installed tools show an `installer.section.installed` badge. Tile version metadata is shown on separate `installer.tile.latest` and `installer.tile.installed` rows, rather than as provider kind text. Runtime bundles add a third row: `installer.tile.node` for the Node runtime version, and `installer.tile.python` for the Python runtime version. Their `installer.tile.installed` row remains the manager version (nvm-windows or uv), so update checks compare like with like. When `latestVersionSeen` differs from `installedVersion`, the tool stays in its normal category section, the latest version value is highlighted, and the tile action uses `installer.actions.update`. The page-level header status carries the most recent check time. One-step bundles usually inherit latest/detected version metadata from their child recipe; Node and Python bundles additionally show the detected runtime version once nvm-windows/uv and the managed runtime are present. The tile action button switches between `installer.actions.install` and `installer.actions.update` based on detected install/update state, and opens the app-owned popup dialog — `InstallerToolDialog` — that owns the detail surface (the previous inline expansion has been removed). Tutorial target on the options form inside the dialog: `installer.toolOptions`.

The dialog has three rendering modes:

- **Installed info** — version, install location (when known), provider summary, latest version + last checked timestamp, pin-version checkbox, and an Update banner when `latestVersionSeen` differs from `installedVersion`. Installed tools that expose a local browser interface may also show `installer.dialog.webUi` plus `installer.actions.run` and `installer.actions.openWebUi`; for n8n, `installer.actions.run` starts the app-local npm project and `installer.actions.openWebUi` opens `http://localhost:5678` in the external browser. For Ollama, `installer.actions.run` starts the local Ollama server with app-local model storage and `installer.actions.openWebUi` opens `http://localhost:11434`. Managed server apps may also show `installer.dialog.windowsService` plus `installer.actions.installService` and `installer.actions.removeService`; these helpers use NSSM to register or remove `KKTerm-n8n` / `KKTerm-Ollama` as Windows services and may show UAC. n8n, Ollama, and future service-capable managed server apps declare NSSM as a prerequisite so it is installed before service registration is offered. Footer: `Uninstall` (danger) · `Run`/`Open web UI` (when available) · `Install service`/`Remove service` (when available) · `Update` (when available) · `Close`.
...
- **npm** — `npm ls -g --json --depth=0`. Looks up the package in the top-level `dependencies`. Managed server apps such as n8n use an app-local marker instead.
...
4. On confirm, the backend dispatches per-provider uninstall: `winget uninstall`, `npm uninstall -g`, `Remove-Item -Recurse` on the github-release install directory, `dism /online /disable-feature`, or `wsl --unregister <distro>` for WSL distro shortcuts.
...
- **`installer.options.scope`** — `user` (default) vs `machine`. Winget recipes only. KKTerm passes `--scope user` by default for scoped winget recipes, including prerequisite installs inside bundles. Machine-scope always triggers UAC.
