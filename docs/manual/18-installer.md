...
- **`installer.options.provider`** — winget (default) vs direct download installer. Shown only for winget recipes whose catalog entry declares a canonical `downloadProvider`; choosing download hides winget-only options for that install.
