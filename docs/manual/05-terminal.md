...
- `terminal.opacity` opens a Transparency slider labelled by `terminal.opacityValue`. The default transparency is 5%, so a custom background or the terminal's default Pane backdrop can subtly show through while text stays readable.
