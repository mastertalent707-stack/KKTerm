...
For RDP-specific overlay bugs, remember that ActiveX is a native child HWND and may cover React UI even when the DOM has a higher `z-index`. The expected behavior is snapshot/parking: when Add Connection, screenshot menus, Region selection, transfer-conflict dialogs, or other registered overlays are open, the frontend captures the active RDP host as a transient bitmap, hides/parks the ActiveX HWND, and shows the bitmap beneath the overlay. If an overlay is covered by RDP, first check whether its selector is registered in `src/workspace/nativeOverlay.ts`, then confirm `RemoteDesktopWorkspace` is suppressing visibility before changing z-index, portal placement, or RDP sizing logic.
